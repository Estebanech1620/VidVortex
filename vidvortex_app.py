...
COOKIE_GUIDE_BODIES: dict[str, str] = {
    ...
    "atlas": (
        ...
        "Atlas is Chromium-based. yt-dlp does not expose an “atlas” browser name, so VidVortex loads your Atlas "
        "profile using Chromium’s cookie reader.\n\n"
        "Download tab — cookies-from-browser\n"
        "Pick ChatGPT Atlas under Are cookies uploaded?, stay signed into youtube.com in Atlas, then Load Qualities. "
        "Fully quit Atlas before downloading if you see cookie/database lock errors.\n"
        "If VidVortex cannot find your profile automatically, set environment variable "
        f"{engine.ATLAS_PROFILE_ENV} to your Atlas profile folder (the folder that contains Network/Cookies — "
        "often …\\User Data\\Default on Windows). You can copy “Profile path” from Atlas’s version/about page. "
        f"Or set {engine.ATLAS_USER_DATA_ENV} to the User Data directory (Default profile is used).\n\n"
        + _guide_export_chromium_family(
            "Cookies.txt export from Atlas",
            "Install from Chrome Web Store if Atlas allows extensions.",
        )
    ),
}
...
class VidVortexApp:
    def __init__(self) -> None:
        ...
        _chromium_family = ["chrome", "edge", "brave", "atlas", "opera", "chromium", "vivaldi"]
        _rest = ["firefox"] + _chromium_family
        if platform.system().lower() == "windows":
            _browser_raw = [""] + _rest
        else:
            _browser_raw = [""] + ["chrome", "edge", "firefox", "brave", "atlas", "opera", "chromium", "vivaldi"]
    ...
    def on_load_qualities(self) -> None:
        ...
        except engine.CookieBrowserResolutionError as exc:
            Messagebox.show_error(exc.user_message, "VidVortex", parent=self.root)
            return
