...
class CookieBrowserResolutionError(RuntimeError):
    """User-facing cookie setup error (missing cookies.txt path, Atlas profile not found, etc.)."""
...
ATLAS_PROFILE_ENV = "VIDVORTEX_ATLAS_PROFILE_DIR"
ATLAS_USER_DATA_ENV = "VIDVORTEX_ATLAS_USER_DATA"
...
def _youtube_metadata_gui_summary(details: str) -> str:
    """Short, actionable text for the desktop GUI when -J fails (stderr often hidden from Activity log)."""
    text = (details or "").lower()
    generic = USER_MESSAGE_YOUTUBE_METADATA_FAILED
    if "failed to decrypt with dpapi" in text:
        return (
            "YouTube lookup failed: Windows could not decrypt Chrome / Edge / Brave cookies (DPAPI). "
            "Choosing “Google Chrome” uses Chrome’s cookie database directly — that often breaks on Windows even "
            "if you’re logged into YouTube.\n\n"
            "What usually works: open the YouTube cookies tab, export cookies.txt from Chrome with "
            "“Get cookies.txt LOCALLY”, upload that file, then Load Qualities (upload overrides the browser "
            "dropdown). Or sign into YouTube in Firefox and pick Firefox here.\n\n"
            "Also try: don’t run VidVortex as Administrator; fully quit Chrome; retry."
        )
    if "sign in to confirm you" in text or "cookies-from-browser or --cookies" in text:
        return (
            "YouTube still rejected the session (sign-in / bot check).\n\n"
            "If you already exported cookies: open youtube.com in Chrome, refresh once while logged in, export "
            "a fresh cookies.txt, upload it on the YouTube cookies tab, then Load Qualities. "
            "Update yt-dlp often (`yt-dlp -U`). VPN or datacenter IPs make this worse.\n\n"
            + generic
        )
    if "http error 403" in text or "403" in text and "forbidden" in text:
        return (
            "YouTube returned 403 Forbidden for this request.\n\n"
            "Refresh cookies (export again after visiting youtube.com), update yt-dlp, and avoid VPN if you can.\n\n"
            + generic
        )
    return generic
...
def _yt_dlp_error_hint(details: str) -> str:
    ...
    if "failed to decrypt with dpapi" in text:
        return (
            ...
            "- Run as a normal user (not Administrator), and fully quit Chrome/Edge/Brave/Atlas, "
            "then retry if you must use them."
        )
    if "sign in to confirm you" in text or "cookies-from-browser or --cookies" in text:
        return (
...
def _subprocess_child_defaults() -> dict[str, Any]:
    """Defaults so yt-dlp/ffmpeg children don't attach to the GUI/Terminal tty or flash consoles.

    macOS: GUI-launched apps often have stdin connected to the user's Terminal; child CLIs can inherit that
    and trigger an extra Terminal window. Linux/Windows: stdin=DEVNULL is still appropriate for non-interactive CLI calls.
    """
...
def _chromium_profile_has_cookie_store(profile_dir: Path) -> bool:
    """True if path looks like a Chromium profile dir with a cookie SQLite DB."""
    net = profile_dir / "Network" / "Cookies"
    legacy = profile_dir / "Cookies"
    try:
        return net.is_file() or legacy.is_file()
    except OSError:
        return False


def _iter_atlas_profile_candidates() -> list[Path]:
    """Ordered Atlas profile directory candidates (typically …/User Data/Default)."""
    ...
    explicit_profile = (os.environ.get(ATLAS_PROFILE_ENV) or "").strip()
    if explicit_profile:
        add(Path(explicit_profile))
        return candidates

    explicit_user_data = (os.environ.get(ATLAS_USER_DATA_ENV) or "").strip()
    if explicit_user_data:
        add(Path(explicit_user_data) / "Default")
        return candidates
    ...
    if system == "windows":
        ...
        if local:
            base = Path(local)
            user_data_roots = [
                base / "OpenAI" / "ChatGPT Atlas" / "User Data",
                base / "OpenAI" / "Atlas" / "User Data",
                base / "ChatGPT Atlas" / "User Data",
                base / "Atlas" / "User Data",
            ]
            for ud in user_data_roots:
                add(ud / "Default")

    elif system == "darwin":
        app_support = home / "Library" / "Application Support"
        user_data_roots = [
            app_support / "OpenAI" / "ChatGPT Atlas" / "User Data",
            app_support / "ChatGPT Atlas" / "User Data",
            app_support / "Atlas" / "User Data",
        ]
        for ud in user_data_roots:
            add(ud / "Default")
        ...
        user_data_roots = [
            xdg_config / "OpenAI" / "ChatGPT Atlas" / "User Data",
            xdg_config / "chatgpt-atlas" / "User Data",
            xdg_data / "OpenAI" / "ChatGPT Atlas" / "User Data",
            xdg_data / "ChatGPT Atlas" / "User Data",
        ]
        for ud in user_data_roots:
            add(ud / "Default")
...
def resolve_atlas_profile_directory() -> Path | None:
    """Resolve ChatGPT Atlas Chromium profile dir containing Network/Cookies, if present."""
    for candidate in _iter_atlas_profile_candidates():
        if _chromium_profile_has_cookie_store(candidate):
            try:
...
def resolve_ytdlp_cookies_from_browser_arg(browser: str | None) -> str | None:
    """Return yt-dlp --cookies-from-browser value, or None if unset.

    Maps ``atlas`` to ``chromium:<profile_path>`` because yt-dlp does not ship an ``atlas`` browser key.
    """
    if browser is None:
        return None
    ...
    if raw.lower() != "atlas":
        return raw
    profile = resolve_atlas_profile_directory()
    if profile is None:
        raise CookieBrowserResolutionError(
            "Could not find the ChatGPT Atlas profile for cookies-from-browser. "
            "Sign into youtube.com in Atlas, then fully quit Atlas and try again. "
            f"If VidVortex still cannot find it, set {ATLAS_PROFILE_ENV} to your Atlas profile folder "
            "(the folder that contains Network/Cookies — often …\\User Data\\Default on Windows). "
            "Copy the path from Atlas’s version/about page if unsure. "
            f"You can also set {ATLAS_USER_DATA_ENV} to the User Data folder (VidVortex will use the Default profile). "
            "Alternatively, export cookies.txt from Atlas and upload it on the YouTube cookies tab."
        )
...
def append_yt_dlp_cookie_flags(cmd: list[str], args: Any) -> list[str]:
    ...
    if raw_file:
        ...
        if raw_str:
            ...
            raise CookieBrowserResolutionError(
                "Your saved YouTube cookies.txt path no longer exists or is not a file:\n"
                f"{path}\n\n"
                "VidVortex will not silently fall back to browser cookies (that mismatch looks like “Chrome "
                "never worked”). On the YouTube cookies tab, choose your cookies.txt again, or clear the file "
                "and pick a browser under Are cookies uploaded?."
            )
    ...
    if browser:
        ...
        if b and b.lower() not in ("none", "off"):
            resolved = resolve_ytdlp_cookies_from_browser_arg(b)
            if resolved:
                return cmd + ["--cookies-from-browser", resolved]
    return cmd
    ...
    *,
    ...
    if result.returncode != 0:
        ...
        raise MetadataFetchError(
            technical,
            user_summary=_youtube_metadata_gui_summary(details),
            silent_gui_log=True,
        )
    try:
...
def open_log_file(path: Path) -> None:
    try:
        if sys.platform.startswith("win"):
            ...
            subprocess.Popen(["open", str(path)], **_subprocess_child_defaults())
        else:
            subprocess.Popen(["xdg-open", str(path)], **_subprocess_child_defaults())
    except Exception:
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=None,
        metavar="BROWSER",
        help="yt-dlp: load cookies from browser profile (e.g. chrome, firefox, edge, atlas)",
    )
    parser.add_argument(
        "--cookies-file",
        dest="cookies_file",
...
def main() -> None:
    ...
    except CookieBrowserResolutionError as exc:
        logging.error("%s", exc.user_message)
        sys.exit(1)
    start = time.time()
